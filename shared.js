    ...
    for (const [_, stat] of stats) {
      if (stat.kind == "audio" && stat.trackIdentifier) {
        const transceiver = pc
          .getTransceivers()
          .find((t) => t.receiver.track.id == stat.trackIdentifier);
...
async function onBroadcastEvent(event, viewer) {
  ...
  if (event.name === "active") {
    ...
    const trackAudio = event.data.tracks?.find(
      ({ media }) => media === "audio"
    );
    if (trackAudio) {
    ...
    const trackVideo = event.data.tracks?.find(
      ({ media }) => media === "video"
    );
    if (trackVideo) {
    ...
    const participantElement = document.getElementById(
      `participant-${event.data.sourceId}`
    );
